...
#define DEVICE_NAME "woo"
#define DEVICE_MAJOR   199 
...
unsigned char FND_DATA_TBL[]={
	0xC0,0xF9,0xA4,0x94,0x99,0x92,0x82,0xF8,0x80,0x90,0xff,
};
...
void NUM_DATA(unsigned char val)
{
...
static int ieb_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	return 0;
}
int ieb_open(struct inode *minode, struct file *mfile)
{
	gpio_init();
...
static ssize_t ieb_read (struct file *file, const char *buf, size_t count, loff_t *f_pos)
{
	gpio_init();
	write_addr_l(TACT);
...
static int ieb_release(struct inode *minode, struct file *mfile)
{
	remove_timer();
	gpio_init();
...
static ssize_t ieb_write (struct file *file, const char *buf, size_t count, loff_t *f_pos)
{
	write_addr_l(FND);
	memset(disp, 0, 4);
...
static struct file_operations ieb_fops = {
   .write      =   ieb_write,
   .read	   =   ieb_read,
   .open       =   ieb_open,
   .release    =   ieb_release,
   .unlocked_ioctl      =   ieb_ioctl,
};

static int __init ieb_init(void){
	int result;
	struct class *ieb_class;
	struct device *dev;

	result = register_chrdev(DEVICE_MAJOR, DEVICE_NAME, &ieb_fops);
	if (result < 0) {
	...
	ieb_class = class_create(THIS_MODULE, DEVICE_NAME);
	dev = device_create(ieb_class, NULL, MKDEV(DEVICE_MAJOR,0), NULL, DEVICE_NAME);
...
static void __exit ieb_exit(void){
	unregister_chrdev(DEVICE_MAJOR, DEVICE_NAME);
	gpio_init();
}

MODULE_LICENSE("GPL");
module_init(ieb_init);
module_exit(ieb_exit);
